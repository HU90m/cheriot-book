...
\hugo{ Was it mentioned that the reader should assume paths (unless specified) are are relative to cheriot-rtos/sdk? }

If a thread entry point returns then the thread exits but, importantly, this does \textem{not} free the thread's stack or trusted stack.
...
\hugo{ Felt like a self referential definition. }
...
\hugo{ I would have understood this example faster, if an ', Iteration: \%d' appended to each print. }
...
This means that a timeout is \textem{stateful}.
...
\hugo{In both cases, even if no other threads are runnable, you have no useful work to do for a bit.}
\hugo{ nitpick }
...
\hugo{
	I don't quite follow the reason the elapsed cycles is higher when threads aren't allowed to wake early.
	Is it because the scheduler is tickless, and so with ThreadSleepNoEarlyWake there are fewer timer interrupts consuming cycles?
}
...
This allows the uncontented case to avoid any cross-compartment calls.
\hugo{ Why are three values needed here? Why are two not be sufficient? }
...
\hugo{ two 'and's in sentence, added comma for readibility.}
...
\hugo{ Could add a reference to "Ensuring heap objects are not deallocated" section here, for people wonding what short-lived claims are. }
...
\hugo{ Is the value odd because the allocator also increment the top 16 bits after reading the list as well as before? }
...
\hugo{ The function declaration uses _Bool as oppose to bool. May as well use stdbool.h like you are stdint.h }
...
\hugo{ This function doc box has a layout overflow in the draft book. }
